...
public class CheckPairs : MonoBehaviour
{
    ...
    bool CheckDirections1(Tile firstTile, Tile secondTile, List<Tile> EmptyTiles, Vector3 secondTilePos)
    {
        ...
        for (int i = x + 1; i < board.gridWidth; i++)
        {
            ...
            {
                SpawnLine(firstTile.transform.position,secondTile.transform.position);
                return true;
            }
    ...
    bool CheckDirections2(Tile firstTile, Tile secondTile, List<Tile> firstList, List<Tile> EmptyTiles, Vector3 firstTilePos)
    {
        ...
        for (int i = x + 1; i < board.gridWidth; i++)
        {
            ...
            {
                SpawnLine(firstTile.transform.position,board.tiles[i,y].transform.position);
                SpawnLine(board.tiles[i,y].transform.position,secondTile.transform.position);
                return true;
            }
            else if (board.tiles[i, y].isEmpty != true)
            {
        ...
        for (int i = y + 1; i < board.gridHeight; i++)
        {
            ...
            {
                SpawnLine(firstTile.transform.position,board.tiles[x,i].transform.position);
                SpawnLine(board.tiles[x,i].transform.position,secondTile.transform.position);
                return true;
            }
            else if (board.tiles[x, i].isEmpty != true)
            {
    ...
    bool CheckDirections3(List<Tile> firstList, List<Tile> secondList,Vector3 firstTilePos, Vector3 secondTilePos)
    {
        foreach (Tile tile in firstList)
        {
            ...
            for (int i = x + 1; i < board.gridWidth; i++)
            {
                ...
                {
                    SpawnLine(firstTilePos,tile.transform.position);
                    SpawnLine(tile.transform.position,board.tiles[i,y].transform.position);
                    SpawnLine(board.tiles[i,y].transform.position,secondTilePos);
                    return true;
                }
                else if (board.tiles[i, y].isEmpty != true)
                {
            ...
            for (int i = y + 1; i < board.gridHeight; i++)
            {
                ...
                {
                    SpawnLine(firstTilePos,tile.transform.position);
                    SpawnLine(tile.transform.position,board.tiles[x,i].transform.position);
                    SpawnLine(board.tiles[x,i].transform.position,secondTilePos);
                    return true;
                }
                else if (board.tiles[x, i].isEmpty != true)
                {
    ...
    void SpawnLine(Vector3 startPoint,Vector3 endPoint)
    {
